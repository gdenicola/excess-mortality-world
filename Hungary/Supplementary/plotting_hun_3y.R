...
#import expected mortality for hun (produced with "expected_country.R"):
exp.hun.avg <- read.table("Data/Data_hun_3y")


#preprocess data for plotting with ggplot
exp.hun.avg.grouped <- group_by(exp.hun.avg,year)
exp.hun.avg.grouped <- summarise(exp.hun.avg.grouped,sum(expected))
...
df_all_ungrouped <- cbind.data.frame(Year,Deaths = exp.hun.avg.grouped$`sum(expected)`
            )
...
#produce plot
plot_hun <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - hun") +
...
ggsave("hun_3y.png",width = 10,height = 7)
