...
#import expected mortality for Sweden (produced with "expected_country.R"):
exp.Sweden.avg <- read.table("Data/Data_Sweden_avg")
exp.Sweden.2015 <- read.table("Data/Data_Sweden_2015")
exp.Sweden.2019 <- read.table("Data/Data_Sweden_2019")


#preprocess data for plotting with ggplot
exp.Sweden.avg.grouped <- group_by(exp.Sweden.avg,year)
exp.Sweden.avg.grouped <- summarise(exp.Sweden.avg.grouped,sum(expected))

exp.Sweden.2015.grouped <- group_by(exp.Sweden.2015,year)
exp.Sweden.2015.grouped <- summarise(exp.Sweden.2015.grouped,sum(expected))

exp.Sweden.2019.grouped <- group_by(exp.Sweden.2019,year)
exp.Sweden.2019.grouped <- summarise(exp.Sweden.2019.grouped,sum(expected))




#input observed mortality figures by year (2015 to 2021):
ist.tod <-  c(90907, 90982,91972,92185,88766,98124,91958)
...
df_all_ungrouped <- cbind.data.frame(Year,Expected = exp.Sweden.avg.grouped$`sum(expected)`,
                                 exp.upper = exp.Sweden.2015.grouped$`sum(expected)`,
                                 exp.lower = exp.Sweden.2019.grouped$`sum(expected)`
            )
...
#produce plot
plot_Sweden <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - Sweden") +
    scale_y_continuous(breaks = seq(70000,110000, 5000),limits = c(79000,105000)) +
...
ggsave("Sweden.png",width = 10,height = 7)
