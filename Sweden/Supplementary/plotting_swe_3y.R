...
#import expected mortality for swe (produced with "expected_country.R"):
exp.swe.avg <- read.table("Data/Data_swe_3y")


#preprocess data for plotting with ggplot
exp.swe.avg.grouped <- group_by(exp.swe.avg,year)
exp.swe.avg.grouped <- summarise(exp.swe.avg.grouped,sum(expected))
...
df_all_ungrouped <- cbind.data.frame(Year,Deaths = exp.swe.avg.grouped$`sum(expected)`
            )
...
#produce plot
plot_swe <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - swe") +
...
ggsave("swe_3y.png",width = 10,height = 7)
