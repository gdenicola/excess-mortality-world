...
#import expected mortality for blg (produced with "expected_country.R"):
exp.blg.avg <- read.table("Data/Data_blg_3y")


#preprocess data for plotting with ggplot
exp.blg.avg.grouped <- group_by(exp.blg.avg,year)
exp.blg.avg.grouped <- summarise(exp.blg.avg.grouped,sum(expected))
...
df_all_ungrouped <- cbind.data.frame(Year,Deaths = exp.blg.avg.grouped$`sum(expected)`
            )
...
#produce plot
plot_blg <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - blg") +
...
ggsave("blg_3y.png",width = 10,height = 7)
