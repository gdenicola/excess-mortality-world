...
#import expected mortality for ita (produced with "expected_country.R"):
exp.ita.avg <- read.table("Data/Data_ita_3y")


#preprocess data for plotting with ggplot
exp.ita.avg.grouped <- group_by(exp.ita.avg,year)
exp.ita.avg.grouped <- summarise(exp.ita.avg.grouped,sum(expected))
...
df_all_ungrouped <- cbind.data.frame(Year,Deaths = exp.ita.avg.grouped$`sum(expected)`
            )
...
#produce plot
plot_ita <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - ita") +
...
ggsave("ita_3y.png",width = 10,height = 7)
