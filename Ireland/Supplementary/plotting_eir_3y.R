...
#import expected mortality for eir (produced with "expected_country.R"):
exp.eir.avg <- read.table("Data/Data_eir_3y")


#preprocess data for plotting with ggplot
exp.eir.avg.grouped <- group_by(exp.eir.avg,year)
exp.eir.avg.grouped <- summarise(exp.eir.avg.grouped,sum(expected))
...
df_all_ungrouped <- cbind.data.frame(Year,Deaths = exp.eir.avg.grouped$`sum(expected)`
            )
...
#produce plot
plot_eir <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - eir") +
...
ggsave("eir_3y.png",width = 10,height = 7)
