...
#import expected mortality for uk (produced with "expected_country.R"):
exp.uk.avg <- read.table("Data/Data_uk_3y")


#preprocess data for plotting with ggplot
exp.uk.avg.grouped <- group_by(exp.uk.avg,year)
exp.uk.avg.grouped <- summarise(exp.uk.avg.grouped,sum(expected))
...
df_all_ungrouped <- cbind.data.frame(Year,Deaths = exp.uk.avg.grouped$`sum(expected)`
            )
...
#produce plot
plot_uk <- ggplot(df_all_ungrouped, aes(Year)) + 
...
ggsave("uk_3y.png",width = 10,height = 7)
