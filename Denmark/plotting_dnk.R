...
#import expected mortality for Denmark (produced with "expected_country.R"):
exp.Denmark.avg <- read.table("Data/Data_Denmark_avg")
exp.Denmark.2015 <- read.table("Data/Data_Denmark_2015")
exp.Denmark.2019 <- read.table("Data/Data_Denmark_2019")


#preprocess data for plotting with ggplot
exp.Denmark.avg.grouped <- group_by(exp.Denmark.avg,year)
exp.Denmark.avg.grouped <- summarise(exp.Denmark.avg.grouped,sum(expected))

exp.Denmark.2015.grouped <- group_by(exp.Denmark.2015,year)
exp.Denmark.2015.grouped <- summarise(exp.Denmark.2015.grouped,sum(expected))

exp.Denmark.2019.grouped <- group_by(exp.Denmark.2019,year)
exp.Denmark.2019.grouped <- summarise(exp.Denmark.2019.grouped,sum(expected))




#input observed mortality figures by year (2015 to 2021):
ist.tod <- c(52555, 52824, 53261, 55232, 53958, 54645, 57152)
...
df_all_ungrouped <- cbind.data.frame(Year,Expected = exp.Denmark.avg.grouped$`sum(expected)`,
                                 exp.upper = exp.Denmark.2015.grouped$`sum(expected)`,
                                 exp.lower = exp.Denmark.2019.grouped$`sum(expected)`
            )
...
#produce plot
plot_Denmark <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - Denmark") +
    scale_y_continuous(breaks = seq(40000,70000, 5000),limits = c(44610.13,65415.2)) +
...
ggsave("Denmark.png",width = 10,height = 7)
