...
#import expected mortality for dnk (produced with "expected_country.R"):
exp.dnk.avg <- read.table("Data/Data_dnk_3y")


#preprocess data for plotting with ggplot
exp.dnk.avg.grouped <- group_by(exp.dnk.avg,year)
exp.dnk.avg.grouped <- summarise(exp.dnk.avg.grouped,sum(expected))
...
df_all_ungrouped <- cbind.data.frame(Year,Deaths = exp.dnk.avg.grouped$`sum(expected)`
            )
...
#produce plot
plot_dnk <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - dnk") +
...
ggsave("dnk_3y.png",width = 10,height = 7)
