...
#import expected mortality for fra (produced with "expected_country.R"):
exp.fra.avg <- read.table("Data/Data_fra_3y")


#preprocess data for plotting with ggplot
exp.fra.avg.grouped <- group_by(exp.fra.avg,year)
exp.fra.avg.grouped <- summarise(exp.fra.avg.grouped,sum(expected))
...
df_all_ungrouped <- cbind.data.frame(Year,Deaths = exp.fra.avg.grouped$`sum(expected)`
            )
...
#produce plot
plot_fra <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - fra") +
...
ggsave("fra_3y.png",width = 10,height = 7)
