...
lifeTable_avg <- read.xls("Data/mortality_fra_avg.xlsx")
lifeTable_2015 <- read.xls("Data/mortality_fra_2015.xlsx")
lifeTable_2019 <- read.xls("Data/mortality_fra_2019.xlsx")
...
data <- read.xls("Data/population_fra.xlsx", header= TRUE, nrows=111 )
data %>% select(1:3)-> data
colnames(data) <- columnnames
#data[,1] <- "2016"

data.2 <- read.xls("Data/population_fra.xlsx", header= TRUE, skip=111, nrows=111 )
data.2 %>% select(1:3)-> data.2
colnames(data.2) <- columnnames
#data.2[,1] <- "2017"
data <- rbind(data, data.2)

data.2 <- read.xls("Data/population_fra.xlsx", header= TRUE, skip=222, nrows=111 )
data.2 %>% select(1:3)-> data.2
colnames(data.2) <- columnnames
#data.2[,1] <- "2018"
data <- rbind(data, data.2)

data.2 <- read.xls("Data/population_fra.xlsx", header= TRUE, skip=333, nrows=111 )
data.2 %>% select(1:3)-> data.2
colnames(data.2) <- columnnames
#data.2[,1] <- "2019"
data <- rbind(data, data.2)

data.2 <- read.xls("Data/population_fra.xlsx", header= TRUE, skip=444, nrows=111 )
data.2 %>% select(1:3)-> data.2
colnames(data.2) <- columnnames
#data.2[,1] <- "2020"
data <- rbind(data, data.2)

data.2 <- read.xls("Data/population_fra.xlsx", skip=555, nrows=111 )
data.2 %>% select(1:3)-> data.2
colnames(data.2) <- columnnames
#data.2[,1] <- "2021"
data <- rbind(data, data.2)

data.2 <- read.xls("Data/population_fra.xlsx", skip=666, nrows=111 )
data.2 %>% select(1:3)-> data.2
colnames(data.2) <- columnnames
#data.2[,1] <- "2021"
data <- rbind(data, data.2)
...
write.table(file = "Data/Data_France_avg",data_x_new)
...
#produce data file with expected mortality (will be used for plotting)
write.table(file = "Data/Data_France_2015",data_x_new)
...
#produce data file with expected mortality (will be used for plotting)
write.table(file = "Data/Data_France_2019",data_x_new)
