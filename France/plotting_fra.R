...
#import expected mortality for France (produced with "expected_country.R"):
exp.France.avg <- read.table("Data/Data_France_avg")
exp.France.2015 <- read.table("Data/Data_France_2015")
exp.France.2019 <- read.table("Data/Data_France_2019")


#preprocess data for plotting with ggplot
exp.France.avg.grouped <- group_by(exp.France.avg,year)
exp.France.avg.grouped <- summarise(exp.France.avg.grouped,sum(expected))

exp.France.2015.grouped <- group_by(exp.France.2015,year)
exp.France.2015.grouped <- summarise(exp.France.2015.grouped,sum(expected))

exp.France.2019.grouped <- group_by(exp.France.2019,year)
exp.France.2019.grouped <- summarise(exp.France.2019.grouped,sum(expected))




#input observed mortality figures by year (2015 to 2021):
ist.tod <- c(581770, 581073, 593606, 596552, 599408, 654599, 644201)
#source: INED
...
df_all_ungrouped <- cbind.data.frame(Year,Expected = exp.France.avg.grouped$`sum(expected)`,
                                 exp.upper = exp.France.2015.grouped$`sum(expected)`,
                                 exp.lower = exp.France.2019.grouped$`sum(expected)`
            )
...
#produce plot
plot_France <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - France") +
    scale_y_continuous(breaks = seq(500000,750000, 50000),limits = c(489203.5,713805.2)) +
...
ggsave("France.png",width = 10,height = 7)
