...
#import expected mortality for jap (produced with "expected_country.R"):
exp.jap.avg <- read.table("Data/Data_jap_3y")


#preprocess data for plotting with ggplot
exp.jap.avg.grouped <- group_by(exp.jap.avg,year)
exp.jap.avg.grouped <- summarise(exp.jap.avg.grouped,sum(expected))
...
df_all_ungrouped <- cbind.data.frame(Year,Deaths = exp.jap.avg.grouped$`sum(expected)`
            )
...
#produce plot
plot_jap <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - jap") +
...
ggsave("jap_3y.png",width = 10,height = 7)
