...
#import expected mortality for lux (produced with "expected_country.R"):
exp.lux.avg <- read.table("Data/Data_lux_3y")


#preprocess data for plotting with ggplot
exp.lux.avg.grouped <- group_by(exp.lux.avg,year)
exp.lux.avg.grouped <- summarise(exp.lux.avg.grouped,sum(expected))
...
df_all_ungrouped <- cbind.data.frame(Year,Deaths = exp.lux.avg.grouped$`sum(expected)`
            )
...
#produce plot
plot_lux <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - lux") +
...
ggsave("lux_3y.png",width = 10,height = 7)
