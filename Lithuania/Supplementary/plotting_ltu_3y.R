...
#import expected mortality for ltu (produced with "expected_country.R"):
exp.ltu.avg <- read.table("Data/Data_ltu_3y")


#preprocess data for plotting with ggplot
exp.ltu.avg.grouped <- group_by(exp.ltu.avg,year)
exp.ltu.avg.grouped <- summarise(exp.ltu.avg.grouped,sum(expected))
...
df_all_ungrouped <- cbind.data.frame(Year,Deaths = exp.ltu.avg.grouped$`sum(expected)`
            )
...
#produce plot
plot_ltu <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - ltu") +
...
ggsave("ltu_3y.png",width = 10,height = 7)
