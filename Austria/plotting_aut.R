...
#import expected mortality for Austria (produced with "expected_country.R"):
exp.Austria.avg <- read.table("Data/Data_Austria_avg")
exp.Austria.2015 <- read.table("Data/Data_Austria_2015")
exp.Austria.2019 <- read.table("Data/Data_Austria_2019")


#preprocess data for plotting with ggplot
exp.Austria.avg.grouped <- group_by(exp.Austria.avg,year)
exp.Austria.avg.grouped <- summarise(exp.Austria.avg.grouped,sum(expected))

exp.Austria.2015.grouped <- group_by(exp.Austria.2015,year)
exp.Austria.2015.grouped <- summarise(exp.Austria.2015.grouped,sum(expected))

exp.Austria.2019.grouped <- group_by(exp.Austria.2019,year)
exp.Austria.2019.grouped <- summarise(exp.Austria.2019.grouped,sum(expected))
...
df_all_ungrouped <- cbind.data.frame(Year,Expected = exp.Austria.avg.grouped$`sum(expected)`,
                                 exp.upper = exp.Austria.2015.grouped$`sum(expected)`,
                                 exp.lower = exp.Austria.2019.grouped$`sum(expected)`
            )
...
#produce plot
plot_Austria <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - Austria") +
...
ggsave("Austria.png",width = 10,height = 7)
