...
#import expected mortality for nor (produced with "expected_country.R"):
exp.nor.avg <- read.table("Data/Data_nor_3y")


#preprocess data for plotting with ggplot
exp.nor.avg.grouped <- group_by(exp.nor.avg,year)
exp.nor.avg.grouped <- summarise(exp.nor.avg.grouped,sum(expected))
...
df_all_ungrouped <- cbind.data.frame(Year,Deaths = exp.nor.avg.grouped$`sum(expected)`
            )
...
#produce plot
plot_nor <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - nor") +
...
ggsave("nor_3y.png",width = 10,height = 7)
