...
#import expected mortality for esp (produced with "expected_country.R"):
exp.esp.avg <- read.table("Data/Data_esp_3y")


#preprocess data for plotting with ggplot
exp.esp.avg.grouped <- group_by(exp.esp.avg,year)
exp.esp.avg.grouped <- summarise(exp.esp.avg.grouped,sum(expected))
...
df_all_ungrouped <- cbind.data.frame(Year,Deaths = exp.esp.avg.grouped$`sum(expected)`
            )
...
#produce plot
plot_esp <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - esp") +
...
ggsave("esp_3y.png",width = 10,height = 7)
