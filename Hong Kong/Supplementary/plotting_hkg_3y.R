...
#import expected mortality for hkg (produced with "expected_country.R"):
exp.hkg.avg <- read.table("Data/Data_hkg_3y")


#preprocess data for plotting with ggplot
exp.hkg.avg.grouped <- group_by(exp.hkg.avg,year)
exp.hkg.avg.grouped <- summarise(exp.hkg.avg.grouped,sum(expected))
...
df_all_ungrouped <- cbind.data.frame(Year,Deaths = exp.hkg.avg.grouped$`sum(expected)`
            )
...
#produce plot
plot_hkg <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - hkg") +
...
ggsave("hkg_3y.png",width = 10,height = 7)
