...
#import expected mortality for SouthKorea (produced with "expected_country.R"):
exp.SouthKorea.avg <- read.table("Data/Data_SouthKorea_avg")
exp.SouthKorea.2015 <- read.table("Data/Data_SouthKorea_2015")
exp.SouthKorea.2019 <- read.table("Data/Data_SouthKorea_2019")


#preprocess data for plotting with ggplot
exp.SouthKorea.avg.grouped <- group_by(exp.SouthKorea.avg,year)
exp.SouthKorea.avg.grouped <- summarise(exp.SouthKorea.avg.grouped,sum(expected))

exp.SouthKorea.2015.grouped <- group_by(exp.SouthKorea.2015,year)
exp.SouthKorea.2015.grouped <- summarise(exp.SouthKorea.2015.grouped,sum(expected))

exp.SouthKorea.2019.grouped <- group_by(exp.SouthKorea.2019,year)
exp.SouthKorea.2019.grouped <- summarise(exp.SouthKorea.2019.grouped,sum(expected))
...
df_all_ungrouped <- cbind.data.frame(Year,Expected = exp.SouthKorea.avg.grouped$`sum(expected)`,
                                 exp.upper = exp.SouthKorea.2015.grouped$`sum(expected)`,
                                 exp.lower = exp.SouthKorea.2019.grouped$`sum(expected)`
            )
...
#produce plot
plot_SouthKorea <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - South Korea") +
...
ggsave("SouthKorea.png",width = 10,height = 7)
