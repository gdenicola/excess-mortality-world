...
lifeTable_avg <- read.xls("Data/mortality_nzl_avg.xlsx")
lifeTable_2015 <- read.xls("Data/mortality_nzl_2015.xlsx")
lifeTable_2016 <- read.xls("Data/mortality_nzl_2016.xlsx")
...
Hazard_2016 <- lifeTable_2016$q.x
...
#here we make the adjustment for the 2016 table
Hazard2_2016 <- cbind(c(lifeTable_2016$q.x[1],
                       (1/2*(lifeTable_2016$q.x[2:110])+
                          1/2*(lifeTable_2016$q.x[3:111]))))
...
Hazard2_2016 <- rbind(Hazard2_2016,
                 c(lifeTable_2016$q.x[111]))
...
#(only for that year). We have to do this for 2015, 2016, 2017, 2018, 2016, 2020 and 2021. 
#Then bind every resulting data.frame to the next one so that all the information 
#is included in only one data.frame.

data <- read.xls("Data/population_nzl.xlsx", header= TRUE, nrows=111 )
data %>% select(1:3)-> data
colnames(data) <- columnnames
#data[,1] <- "2016"

data.2 <- read.xls("Data/population_nzl.xlsx", header= TRUE, skip=111, nrows=111 )
data.2 %>% select(1:3)-> data.2
colnames(data.2) <- columnnames
#data.2[,1] <- "2017"
data <- rbind(data, data.2)

data.2 <- read.xls("Data/population_nzl.xlsx", header= TRUE, skip=222, nrows=111 )
data.2 %>% select(1:3)-> data.2
colnames(data.2) <- columnnames
#data.2[,1] <- "2018"
data <- rbind(data, data.2)

data.2 <- read.xls("Data/population_nzl.xlsx", header= TRUE, skip=333, nrows=111 )
data.2 %>% select(1:3)-> data.2
colnames(data.2) <- columnnames
#data.2[,1] <- "2016"
data <- rbind(data, data.2)

data.2 <- read.xls("Data/population_nzl.xlsx", header= TRUE, skip=444, nrows=111 )
data.2 %>% select(1:3)-> data.2
colnames(data.2) <- columnnames
#data.2[,1] <- "2020"
data <- rbind(data, data.2)

data.2 <- read.xls("Data/population_nzl.xlsx", skip=555, nrows=111 )
data.2 %>% select(1:3)-> data.2
colnames(data.2) <- columnnames
#data.2[,1] <- "2021"
data <- rbind(data, data.2)

data.2 <- read.xls("Data/population_nzl.xlsx", skip=666, nrows=111 )
data.2 %>% select(1:3)-> data.2
colnames(data.2) <- columnnames
#data.2[,1] <- "2021"
data <- rbind(data, data.2)
...
Hazard2_2016_df <- as.data.frame(Hazard2_2016)
...
#Calculation of average expected mortality (2015-2016)
...
data_x[445:555,2] * Hazard2_avg_df[1:111,1] -> exp2016
exp_both <- rbind(exp_both, exp2016)
...
write.table(file = "Data/Data_NewZealand_avg",data_x_new)
...
data_x[445:555,2] * Hazard2_2015_df[1:111,1] -> exp2016
exp_both <- rbind(exp_both, exp2016)
...
#produce data file with expected mortality (will be used for plotting)
write.table(file = "Data/Data_NewZealand_2015",data_x_new)




#Calculation of lower expected mortality bound (2016 only)

data_x[1:111,2] * Hazard2_2016_df[1:111,1] -> exp_both

data_x[112:222,2] * Hazard2_2016_df[1:111,1] -> exp2016
exp_both <- rbind(exp_both, exp2016)

data_x[223:333,2] * Hazard2_2016_df[1:111,1] -> exp2017
exp_both <- rbind(exp_both, exp2017)

data_x[334:444,2] * Hazard2_2016_df[1:111,1] -> exp2018
exp_both <- rbind(exp_both, exp2018)

data_x[445:555,2] * Hazard2_2016_df[1:111,1] -> exp2016
exp_both <- rbind(exp_both, exp2016)

data_x[556:666,2] * Hazard2_2016_df[1:111,1] -> exp2020
exp_both <- rbind(exp_both, exp2020)

data_x[667:777,2] * Hazard2_2016_df[1:111,1] -> exp2021
exp_both <- rbind(exp_both, exp2021)
...
#produce data file with expected mortality (will be used for plotting)
write.table(file = "Data/Data_NewZealand_2016",data_x_new)
