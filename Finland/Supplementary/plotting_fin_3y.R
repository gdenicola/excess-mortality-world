...
#import expected mortality for fin (produced with "expected_country.R"):
exp.fin.avg <- read.table("Data/Data_fin_3y")


#preprocess data for plotting with ggplot
exp.fin.avg.grouped <- group_by(exp.fin.avg,year)
exp.fin.avg.grouped <- summarise(exp.fin.avg.grouped,sum(expected))
...
df_all_ungrouped <- cbind.data.frame(Year,Deaths = exp.fin.avg.grouped$`sum(expected)`
            )
...
#produce plot
plot_fin <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - fin") +
...
ggsave("fin_3y.png",width = 10,height = 7)
