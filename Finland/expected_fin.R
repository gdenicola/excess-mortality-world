...
lifeTable_avg <- read.xls("Data/mortality_fin_avg.xlsx")
lifeTable_2016 <- read.xls("Data/mortality_fin_2016.xlsx") #2016 had higher overall death rates than 2016 in Finland
lifeTable_2019 <- read.xls("Data/mortality_fin_2019.xlsx")
...
#(only for that year). We have to do this for 2016, 2016, 2017, 2018, 2019, 2020 and 2021. 
#Then bind every resulting data.frame to the next one so that all the information 
#is included in only one data.frame.

data <- read.xls("Data/population_fin.xlsx", header= TRUE, nrows=111 )
data %>% select(1:3)-> data
colnames(data) <- columnnames
#data[,1] <- "2016"

data.2 <- read.xls("Data/population_fin.xlsx", header= TRUE, skip=111, nrows=111 )
data.2 %>% select(1:3)-> data.2
colnames(data.2) <- columnnames
#data.2[,1] <- "2017"
data <- rbind(data, data.2)

data.2 <- read.xls("Data/population_fin.xlsx", header= TRUE, skip=222, nrows=111 )
data.2 %>% select(1:3)-> data.2
colnames(data.2) <- columnnames
#data.2[,1] <- "2018"
data <- rbind(data, data.2)

data.2 <- read.xls("Data/population_fin.xlsx", header= TRUE, skip=333, nrows=111 )
data.2 %>% select(1:3)-> data.2
colnames(data.2) <- columnnames
#data.2[,1] <- "2019"
data <- rbind(data, data.2)

data.2 <- read.xls("Data/population_fin.xlsx", header= TRUE, skip=444, nrows=111 )
data.2 %>% select(1:3)-> data.2
colnames(data.2) <- columnnames
#data.2[,1] <- "2020"
data <- rbind(data, data.2)

data.2 <- read.xls("Data/population_fin.xlsx", skip=555, nrows=111 )
data.2 %>% select(1:3)-> data.2
colnames(data.2) <- columnnames
#data.2[,1] <- "2021"
data <- rbind(data, data.2)

data.2 <- read.xls("Data/population_fin.xlsx", skip=666, nrows=111 )
data.2 %>% select(1:3)-> data.2
colnames(data.2) <- columnnames
#data.2[,1] <- "2021"
data <- rbind(data, data.2)
...
#Calculation of average expected mortality (2016-2019)
...
write.table(file = "Data/Data_Finland_avg",data_x_new)





#Calculation of upper expected mortality bound (2016 only)
...
data_x[445:555,2] * Hazard2_2016_df[1:111,1] -> exp2019
exp_both <- rbind(exp_both, exp2019)
...
#produce data file with expected mortality (will be used for plotting)
write.table(file = "Data/Data_Finland_2016",data_x_new)
...
#produce data file with expected mortality (will be used for plotting)
write.table(file = "Data/Data_Finland_2019",data_x_new)
