...
#import expected mortality for cze (produced with "expected_country.R"):
exp.cze.avg <- read.table("Data/Data_cze_3y")


#preprocess data for plotting with ggplot
exp.cze.avg.grouped <- group_by(exp.cze.avg,year)
exp.cze.avg.grouped <- summarise(exp.cze.avg.grouped,sum(expected))
...
df_all_ungrouped <- cbind.data.frame(Year,Deaths = exp.cze.avg.grouped$`sum(expected)`
            )
...
#produce plot
plot_cze <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - cze") +
...
ggsave("cze_3y.png",width = 10,height = 7)
