...
#import expected mortality for twn (produced with "expected_country.R"):
exp.twn.avg <- read.table("Data/Data_twn_3y")


#preprocess data for plotting with ggplot
exp.twn.avg.grouped <- group_by(exp.twn.avg,year)
exp.twn.avg.grouped <- summarise(exp.twn.avg.grouped,sum(expected))
...
df_all_ungrouped <- cbind.data.frame(Year,Deaths = exp.twn.avg.grouped$`sum(expected)`
            )
...
#produce plot
plot_twn <- ggplot(df_all_ungrouped, aes(Year)) + 
    ...
    ggtitle("Expected versus observed yearly deaths - twn") +
...
ggsave("twn_3y.png",width = 10,height = 7)
